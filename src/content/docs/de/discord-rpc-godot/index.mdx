...
<Tabs>
  ...
  <TabItem label="Manuell">
    <Steps>
      <ol>
        <li>
        <DownloadBtn label="Addon herunterladen" href="https://github.com/vaporvee/discord-rpc-godot/releases/latest/download/ADDON-discord-rpc-godot.zip/" /><br/>
        </li>
        <li>
...
<DownloadBtn label="Probier eine fertige Demo" href="https://github.com/vaporvee/discord-rpc-godot/releases/latest/download/Demo-Export.zip" />
## Fehlerbehebung
Versuche erstmal, das Plugin neu zu installieren, und starte sowohl deinen Godot-Editor als auch deinen Discord-Client neu.
<details><summary><b>In meiner Godot-Konsole gibt es keine Fehler vom Plugin, aber meine Aktivität wird nicht in meinem Discord-Client angezeigt.</b></summary>Stelle sicher, dass eine `DiscordSDK.run_callbacks()`-Funktion in einer `_process(delta)`-Funktion ausgeführt wird. Dies sollte im Autoload passieren, das vom Plugin hinzugefügt wurde. Wenn es immer noch nicht hilft, versuche die obige Demo. Wenn in deinem Discord-Profilkarten immer noch nichts angezeigt wird, musst du die entsprechenden Einstellungen in deinem Discord-CLient zurücksetzen oder das plugin neu installieren.</details>
<details>
